...
std::vector<bool> decideBools(int i, int size) { // algorithm sucks a lot
...
void printPercentage(std::vector<str>& breeds) {
	struct Display_string {
		short counter{};
		str suka;
	};

	std::vector<Display_string> percentage{};

	for (int i{}; i < percentage.size(); ++i)
		if (i < percentage.size() - 1)
			if (percentage[i].counter < percentage[i + 1].counter) {
				short psika{ percentage[i].counter };
				percentage[i].counter = percentage[i + 1].counter = psika;
				i = -1;
			}

	for (int i{}; i < percentage.size(); ++i) {
		if (i == percentage.size() - 1)
			std::cout << percentage[i].counter;
		else
			std::cout << percentage[i].counter << ':';
	}
}
...
int main() {
	std::cout << "Welcome to Genotype Reader v1.5a\nCopyright: sirkostya009\nLicense: Apache 2.0\n\n";
